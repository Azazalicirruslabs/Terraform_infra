...
@router.post("/api/interaction-network", tags=["Interactions"])
    ...
    try:
        ...
        try:
            data_preview_response = handle_request(
                model_service.interaction_network, model_overview_payload, top_k, sample_rows
            )
            ...
            raise HTTPException(status_code=500, detail=f"⚠️ Failed to compute interaction network: {str(e)}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"⚠️ Unexpected internal error while performing interaction network analysis: {str(e)}",
        )
