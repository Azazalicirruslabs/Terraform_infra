...
router = APIRouter(prefix="/classification", tags=["Prediction"])


@router.post("/api/individual-prediction", tags=["Prediction"])
async def post_individual_prediction(
    ...
    try:
        ...
        # ✅ Extract and validate instance index
        try:
        ...
        try:
            data_preview_response = handle_request(
                model_service.individual_prediction, model_overview_payload, instance_idx
            )
            ...
            raise HTTPException(status_code=500, detail=f"⚠️ Failed to compute individual prediction: {str(e)}")
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"⚠️ Unexpected internal error while performing individual prediction: {str(e)}"
        )
