...
class MitigationStrategies:
    ...
    def _apply_reweighing(
        ...
        try:
            ...
            dataset = BinaryLabelDataset(df=train_df, label_names=["label"], protected_attribute_names=["sensitive"])

            # Apply Reweighing
            reweighing = Reweighing(unprivileged_groups=[{"sensitive": 0}], privileged_groups=[{"sensitive": 1}])
        ...
        try:
            ...
            if y_test is None or y_pred_test is None:
                return {
                    ...
                    "status": "error",
                    "error": "y_test and y_pred_test required",
                }
            ...
            # Check if model is trainable (has fit method)
            is_trainable = hasattr(model, "fit") and not (
                hasattr(model, "sess") and hasattr(model, "get_inputs")
            )  # Exclude ONNX wrapper if it mimics fit

            # For ONNX or other inference-only models, we MUST use prefit=True
            use_prefit = not is_trainable

            print(f"  Model trainable: {is_trainable} -> Using prefit={use_prefit}")

            optimizer = ThresholdOptimizer(estimator=model, constraints="equalized_odds", prefit=use_prefit)
            ...
            y_pred_optimized = optimizer.predict(
                X_test if X_test is not None else X_train, sensitive_features=sensitive_attr
            )
    ...
    def _fallback_threshold_optimization(
        self, y_train: np.ndarray, y_pred: np.ndarray, sensitive_attr: np.ndarray
    ) -> Dict:
        """
        ...
        try:
            ...
            return {"strategy": "threshold_optimization", "type": "post-processing", "status": "error", "error": str(e)}
        ...
        try:
            ...
            for group in unique_groups:
                ...
                for group in unique_groups:
                    ...
                    tpr_final = (
                        np.mean(group_pred_final[group_y_true == 1] == 1) if np.sum(group_y_true == 1) > 0 else 0.0
                    )
                    fpr_final = (
                        np.mean(group_pred_final[group_y_true == 0] == 1) if np.sum(group_y_true == 0) > 0 else 0.0
                    )

                    group_metrics[group] = {"tpr": tpr_final, "fpr": fpr_final, "threshold": best_threshold}
                    print(
                        f"    Group {group}: threshold={best_threshold:.3f}, TPR={tpr_final:.3f}, FPR={fpr_final:.3f}"
                    )
    ...
    def _apply_fairness_regularization(
        ...
        try:
            ...
            mitigator = ExponentiatedGradient(
                self.user_model, constraints=DemographicParity(), eps=0.01, max_iter=50  # <-- USE YOUR UPLOADED MODEL
            )
    ...
    def _apply_adversarial_debiasing(
        ...
        try:
            ...
            if hasattr(model, "partial_fit"):
                ...
                print(
                    f"    Normalized weights: min={weights.min():.3f}, max={weights.max():.3f}, mean={weights.mean():.3f}"
                )
                ...
                for epoch in range(3):  # CHANGE 5a: Run 3 epochs
                    ...
                    model.partial_fit(X_train, y_train, sample_weight=weights, classes=unique_classes)
                    ...
                    for group in np.unique(sensitive_attr):
                        ...
                        if group_acc < 0.8:  # Threshold for "low accuracy"
                            ...
                            if np.sum(neg_preds_mask) > 0:
                                ...
                                print(
                                    f"    Group {group}: Adjusted {len(flip_indices)} predictions (acc={group_acc:.3f})"
                                )
    ...
    def run_mitigation_strategy(
        ...
        result = strategy_func(X_train, y_train, sensitive_attr, X_test, y_test, y_pred_test)
...
class MitigationPipeline:
    ...
    def __init__(self):
        ...
        self.pipeline_stages = {"pre": [], "in": [], "post": []}
        self.results = {}
    ...
    def execute_pipeline(
        ...
        if self.pipeline_stages["pre"]:
            ...
            for idx, strategy in enumerate(self.pipeline_stages["pre"], 1):
                ...
                pre_results.append({"strategy": strategy, "result": result, "status": result.get("status", "unknown")})
        ...
        if self.pipeline_stages["in"]:
            ...
            for idx, strategy in enumerate(self.pipeline_stages["in"], 1):
                ...
                in_results.append({"strategy": strategy, "result": result, "status": result.get("status", "unknown")})
        ...
        if self.pipeline_stages["post"]:
            ...
            for idx, strategy in enumerate(self.pipeline_stages["post"], 1):
                ...
                post_results.append({"strategy": strategy, "result": result, "status": result.get("status", "unknown")})
    ...
    def get_pipeline_summary(self) -> Dict:
        ...
        return {
            "total_strategies": sum(len(v) for v in self.pipeline_stages.values()),
            "pre_processing": {"count": len(self.pipeline_stages["pre"]), "strategies": self.pipeline_stages["pre"]},
            "in_processing": {"count": len(self.pipeline_stages["in"]), "strategies": self.pipeline_stages["in"]},
            "post_processing": {"count": len(self.pipeline_stages["post"]), "strategies": self.pipeline_stages["post"]},
        }


def _simple_equalized_odds_adjustment(y_pred: np.ndarray, y_true: np.ndarray, sensitive_attr: np.ndarray) -> np.ndarray:
    """
