...
from services.fairness.app.src.mitigation_strategies import MitigationStrategies
...
class PipelineOptimizer:
    ...
    def __init__(self, bias_detector=None, user_model=None):
        ...
        self.pre_strategies = [name for name, info in self.strategies_info.items() if "pre-processing" in info["type"]]
        self.in_strategies = [name for name, info in self.strategies_info.items() if "in-processing" in info["type"]]
        self.post_strategies = [
            name for name, info in self.strategies_info.items() if "post-processing" in info["type"]
        ]
    ...
    def stage_wise_greedy_search(
        ...
        for idx, strategy in enumerate(self.pre_strategies, 1):
            ...
            biased_count = self._evaluate_pipeline_real(test_pipeline, X_train, y_train, sensitive_attr, y_pred)

            improvement = current_biased_count - biased_count
            print(f"      Biased count: {biased_count}, Improvement: {improvement}")

            if improvement > best_pre_improvement or (
                improvement == best_pre_improvement and biased_count < best_pre_biased_count
            ):
        ...
        for idx, strategy in enumerate(self.in_strategies, 1):
            ...
            test_pipeline = {"pre": best_pipeline["pre"].copy(), "in": [strategy], "post": []}
            biased_count = self._evaluate_pipeline_real(test_pipeline, X_train, y_train, sensitive_attr, y_pred)

            improvement = current_biased_count - biased_count
            print(f"      Biased count: {biased_count}, Improvement: {improvement}")

            if improvement > best_in_improvement or (
                improvement == best_in_improvement and biased_count < best_in_biased_count
            ):
        ...
        for idx, strategy in enumerate(self.post_strategies, 1):
            ...
            test_pipeline = {"pre": best_pipeline["pre"].copy(), "in": best_pipeline["in"].copy(), "post": [strategy]}
            biased_count = self._evaluate_pipeline_real(test_pipeline, X_train, y_train, sensitive_attr, y_pred)

            improvement = current_biased_count - biased_count
            print(f"      Biased count: {biased_count}, Improvement: {improvement}")

            if improvement > best_post_improvement or (
                improvement == best_post_improvement and biased_count < best_post_biased_count
            ):
    ...
    def _get_predictions_after_strategy(
        self, strategy: str, X_train: pd.DataFrame, y_train: np.ndarray, sensitive_attr: np.ndarray, y_pred: np.ndarray
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            result = mitigator.run_mitigation_strategy(
                strategy, X_train, y_train, sensitive_attr, X_test=X_train, y_test=y_train, y_pred_test=y_pred
            )

            if mitigator.y_augmented is not None:
                X_train_current = mitigator.X_augmented
                mitigator.y_augmented
                mitigator.sensitive_augmented
                ...
                mitigator.y_pred_adjusted

            elif mitigator.fine_tuned_model is not None:
                mitigator.fine_tuned_model.predict(X_train_current)
    ...
    def top_k_method(
        ...
        final_biased_count = self._evaluate_pipeline_real(combined_pipeline, X_train, y_train, sensitive_attr, y_pred)
    ...
    def brute_force_search(
        ...
        for pre, in_proc, post in product(pre_combos, in_combos, post_combos):
            ...
            pipeline = {"pre": list(pre), "in": list(in_proc), "post": list(post)}
        ...
        for idx, pipeline in enumerate(all_pipelines, 1):
            ...
            biased_count = self._evaluate_pipeline_real(pipeline, X_train, y_train, sensitive_attr, y_pred)

            improvement = baseline_biased_count - biased_count

            results.append({"pipeline": pipeline, "biased_count": int(biased_count), "improvement": float(improvement)})
    ...
    def _evaluate_pipeline_real(
        ...
        try:
                    ...
                    try:
                        ...
                        if mitigator.y_pred_adjusted is not None:
                            ...
                            if hasattr(mitigator, "X_augmented") and mitigator.X_augmented is not None:
                                ...
                                print(
                                    f"        Generated {len(y_pred_mitigated)} predictions (original + augmented copies)"
                                )
                            ...
                            print(
                                f"        Updated all data to size: X={X_train_current.shape[0]}, y={len(y_train_current)}, sensitive={len(sensitive_attr_current)}"
                            )
                            ...
                            try:
                                ...
                                is_onnx = hasattr(temp_model, "sess") or "onnx" in str(type(temp_model)).lower()

                                # Try to retrain with sample weights
                                if hasattr(temp_model, "fit") and not is_onnx:
                                    print(f"        Attempting to retrain with sample weights...")
                                    temp_model.fit(
                                        X_train_current, y_train_current, sample_weight=mitigator.sample_weights
                                    )

                                    # Generate predictions with reweighted model
                                    y_pred_mitigated = temp_model.predict(X_train_current)
                                    print(
                                        f"        ✅ Retrained model with sample weights: {len(y_pred_mitigated)} predictions"
                                    )
                                else:
                                    print(f"        ⚠️ Model is ONNX or not trainable; skipping reweighing retrain.")
            ...
            print(
                f"    Final evaluation sizes: y_train={len(y_train_current)}, y_pred={len(y_pred_mitigated)}, sensitive={len(sensitive_attr_current)}"
            )
