...
class BiasDetector:
    ...
    def calculate_statistical_parity_fairlearn(
        ...
        try:
            ...
            print(
                f"[FAIRLEARN] Statistical Parity - Difference: {dp_diff:.4f}, Threshold: {self.thresholds['statistical_parity']:.4f}"
            )
    ...
    def calculate_disparate_impact_aif360(
        ...
        try:
            if len(y_pred) != len(sensitive_feature):
                raise ValueError(
                    f"Size mismatch: y_pred ({len(y_pred)}) vs sensitive_feature ({len(sensitive_feature)})"
                )
            ...
            print(
                f"[AIF360] Disparate Impact - Ratio: {ratio:.4f}, Threshold: {self.thresholds['disparate_impact']:.4f}"
            )
    ...
    def calculate_equal_opportunity_fairlearn(
        ...
        try:
            ...
            print(
                f"[FAIRLEARN] Equal Opportunity - TPR Diff: {tpr_diff:.4f}, Threshold: {self.thresholds['equal_opportunity']:.4f}"
            )
    ...
    def calculate_equalized_odds_fairlearn(
        ...
        try:
            ...
            eo_diff = equalized_odds_difference(y_true_series, y_pred_series, sensitive_features=sensitive_series)
            ...
            if len(unique_groups) > 2:
                ...
                print(
                    f"[FAIRLEARN] Equalized Odds - Max Diff: {max_diff:.4f}, Threshold: {self.thresholds['equalized_odds']:.4f}"
                )
    ...
    def _equalized_odds_multiclass(self, y_true, y_pred, sensitive_feature):
        ...
        for target_class in classes:
            ...
            try:
                ...
                eo = equalized_odds_difference(y_true_binary, y_pred_binary, sensitive_features=sensitive_feature)
    ...
    def calculate_calibration_aif360(
        self, y_true: np.ndarray, y_pred_proba: np.ndarray, sensitive_feature: np.ndarray
    ) -> Dict:
        """
        ...
        try:
            ...
            if len(y_true) != len(sensitive_feature):
                raise ValueError(
                    f"Size mismatch: y_true ({len(y_true)}) vs sensitive_feature ({len(sensitive_feature)})"
                )
    ...
    def calculate_generalized_entropy_index_aif360(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict:
        """
        ...
        try:
            ...
            print(
                f"[AIF360] Generalized Entropy Index: {gei:.4f}, Threshold: {self.thresholds['generalized_entropy_index']:.4f}"
            )
    ...
    def run_all_metrics(
        ...
        """
        print(f"\n=== Running Bias Detection (Task: {task_type}) ===")

        # Validate input sizes FIRST
        try:
            self._validate_inputs(y_true, y_pred, sensitive_feature)
            print(
                f"[VALIDATION] Input sizes validated: y_true={len(y_true)}, y_pred={len(y_pred)}, sensitive={len(sensitive_feature)}"
            )
        ...
        if task_type == "regression":
            # --- Regression Metrics ---
            print("\n[Regression Mode]")
            results["regression_metrics"] = self._calculate_regression_metrics(y_true, y_pred, sensitive_feature)

            # Add Generalized Entropy Index (applies to regression too usually, but checking implementation)
            # AIF360 GEI implementation often expects inequalities. Let's keep it if possible or wrap it.
            # Assuming GEI is for inequality, we can try it.
            try:
                results["generalized_entropy_index"] = self.calculate_generalized_entropy_index_aif360(y_true, y_pred)
            except:
                pass
            ...
            # 1. Group Performance Comparison (Requested Metrics)
            results["classification_performance"] = self._calculate_classification_performance(y_true, y_pred)
            ...
            results["equal_opportunity"] = self.calculate_equal_opportunity_fairlearn(y_true, y_pred, sensitive_feature)
            results["equalized_odds"] = self.calculate_equalized_odds_fairlearn(y_true, y_pred, sensitive_feature)

            # 3. AIF360 metrics
            print("\n[Using AIF360 Library]")
            results["disparate_impact"] = self.calculate_disparate_impact_aif360(y_true, y_pred, sensitive_feature)
            results["generalized_entropy_index"] = self.calculate_generalized_entropy_index_aif360(y_true, y_pred)

            if y_pred_proba is not None:
                results["calibration"] = self.calculate_calibration_aif360(y_true, y_pred_proba, sensitive_feature)

        # Summary
        biased_metrics = [k for k, v in results.items() if isinstance(v, dict) and v.get("is_biased", False)]
    ...
    def _calculate_classification_performance(self, y_true, y_pred) -> Dict:
        """Calculate requested classification metrics"""
        try:
            # Handle potential multiclass by using 'weighted' average for metrics that require it
            ...
            # Selection Rate: P(Predicted=1) - mostly relevant for binary
            # Base Rate: P(Actual=1)
            if not is_multiclass:
                selection_rate = np.mean(y_pred)
                base_rate = np.mean(y_true)
            else:
                selection_rate = 0.0  # Not well defined for multiclass single number
                base_rate = 0.0

            return {
                "metric": "classification_performance",
                "accuracy": float(acc),
                ...
                "description": "Overall Classification Performance",
            }
        except Exception as e:
            print(f"Error in classification performance: {e}")
            return {"error": str(e)}

    def _calculate_regression_metrics(self, y_true, y_pred, sensitive_feature) -> Dict:
        """Calculate regression metrics: MSE, MAE, R2 and Disparity"""
        try:
            # Overall metrics
            ...
            # Disparity Analysis (Group-wise MSE)
            # Find groups
            ...
            for g in groups:
                ...
                g_mse = mean_squared_error(y_true[mask], y_pred[mask])
                g_mae = mean_absolute_error(y_true[mask], y_pred[mask])
                group_metrics[str(g)] = {"mse": float(g_mse), "mae": float(g_mae)}

                max_mse = max(max_mse, g_mse)
                min_mse = min(min_mse, g_mse)

            # MSE Disparity: Max MSE - Min MSE (or Ratio?)
            # Usually Ratio or Difference. Let's use Difference.
            mse_disparity = max_mse - min_mse

            # Threshold Check? (Heuristic: is disparity > 10% of overall MSE?)
            is_biased = mse_disparity > (0.1 * mse) if mse > 0 else False
            ...
            return {
                ...
                "mae": float(mae),
                "r2_score": float(r2),
                "mse_disparity": float(mse_disparity),
                "group_metrics": group_metrics,
                "is_biased": bool(is_biased),
                "description": "Regression Performance & MSE Disparity",
            }
        except Exception as e:
    ...
    def _validate_inputs(self, y_true: np.ndarray, y_pred: np.ndarray, sensitive_feature: np.ndarray) -> bool:
        ...
        if len(y_true) != len(sensitive_feature):
            raise ValueError(
                f"y_true ({len(y_true)}) and sensitive_feature ({len(sensitive_feature)}) have different lengths"
            )
