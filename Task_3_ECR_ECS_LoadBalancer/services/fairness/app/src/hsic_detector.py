...
from services.fairness.app.src.config import HSIC_THRESHOLD_PERCENTILE
...
class SensitiveFeatureDetector:
    ...
    def _simple_correlation_analysis(
        ...
        for feature in self.feature_names:
            try:
                ...
                print(
                    f"  {feature}: {test_result['test_type']} (p-value: {test_result['p_value']:.4f}, correlation: {test_result['correlation']:.4f})"
                )
    ...
    def get_feature_relationships(self, feature_index: int = 0, num_neighbors: int = 5) -> Dict:
        ...
        try:
            ...
            neighbor_indices = self.hsic_lasso.get_index_neighbors(
                feat_index=feature_index, num_neighbors=num_neighbors
            )
            ...
            neighbors = [
                {"feature": self.feature_names[idx], "relationship_score": float(score)}
                for idx, score in zip(neighbor_indices, neighbor_scores)
            ]

            return {"primary_feature": self.feature_names[feature_index], "related_features": neighbors}
        except Exception as e:
            return {"error": str(e)}
    ...
    def _preprocess_for_hsic(self, X: pd.DataFrame, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        print(
            f"[DEBUG] X range: [{X_processed.min()}, {X_processed.max()}], y range: [{y_processed.min()}, {y_processed.max()}]"
        )
...
class AnalysisEngine:
    ...
    def run_analysis(self, n_top_features: int = 10) -> Dict:
        ...
        sensitive_features, feature_scores, threshold = self.detector.detect_sensitive_features(
            X_train, y_train, n_features_to_select=min(n_top_features, X_train.shape[1])
        )
