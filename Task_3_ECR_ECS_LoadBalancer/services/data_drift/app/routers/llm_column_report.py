...
class LlmColumnRequest(BaseModel):
...
@router.post("/llm_column_report/{analysis_type}/{project_name}/{column}", tags=["Old Data Drift"])
def run_llm_drift_analysis(
    analysis_type: str,
    project_name: str,
    column: str,
    payload: Optional[LlmColumnRequest] = Body(None),
    ...
    try:
        if ref_df is not None:
            html, drift_dict = drift_analyzer.generate_column_drift_report(column)
            summary_text = drift_analyzer.get_column_drift_summary(drift_dict, column, psi_threshold=0.10)
            data = llm_analyzer.analyze_column_drift(summary_text)
