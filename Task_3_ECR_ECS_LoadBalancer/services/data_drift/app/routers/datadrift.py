...
def load_flexible_csv(file_bytes: bytes, filename: str = None) -> pd.DataFrame:
    ...
    if filename and filename.lower().endswith(".parquet"):
        try:
            ...
            raise RuntimeError(f"❌ Could not parse the Parquet file. Reason: {e}")
...
def s3_bucket_data(access_token, analysis_type, project_name):
    # Step 1: Call your FastAPI endpoint to get file URLs
    FILE_MODEL_DOWNLOAD_API = os.getenv("FILES_API_BASE_URL")
    api_url = f"{FILE_MODEL_DOWNLOAD_API}/{analysis_type}/{project_name}"
    # Step 2: Set Authorization header
    headers = {"Authorization": f"Bearer {access_token}"}

    # Step 3: Call FastAPI endpoint with the token
    # api_url = "http://localhost:8000/api/files_download/"
            ...
            for file_info in files:
                ...
                if file_response.status_code == 200:
                    ...
                    try:
                        ...
                        df = load_flexible_csv(file_content, filename=file_name)
...
class DriftAnalyzer:
    ...
    def load_data_from_s3(
        ...
        try:
            data = s3_bucket_data(access_token, analysis_type, project_name)
    ...
    def get_preview_data(self, ref_df, cur_df, n=5):
        # preview_data = {
        #     'ref_preview': ref_df.head(n).to_dict(orient='records'),
        #     'cur_preview': cur_df.head(n).to_dict(orient='records')
        # }
        # Replace NaN and inf values with None so they are JSON serializable
        ref_clean = ref_df.head(n).replace([float("nan"), float("inf"), float("-inf")], None)
        cur_clean = cur_df.head(n).replace([float("nan"), float("inf"), float("-inf")], None)

        preview_data = {
            "ref_preview": ref_clean.to_dict(orient="records"),
            "cur_preview": cur_clean.to_dict(orient="records"),
        }
        return preview_data
    ...
    def split_by_date(self, df: pd.DataFrame, date_col, ref_range: tuple, cur_range: tuple, time_col: str = None):
        # ref_range = ("2004-03-10", "2004-03-11")
        # cur_range = ("2005-04-04", "2005-04-04")
        # print("Splitting by date with ranges:", ref_range, cur_range)
        if time_col:
        ...
        df = df[df["datetime"].notna()]  # Filter out invalid datetimes

        # Extend end boundaries to capture full days
        ref_start_dt = pd.to_datetime(ref_range[0])
        ref_end_dt = pd.to_datetime(ref_range[1]) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)

        cur_start_dt = pd.to_datetime(cur_range[0])
        cur_end_dt = pd.to_datetime(cur_range[1]) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)

        # print(ref_start_dt,ref_end_dt, cur_start_dt, cur_end_dt, "yyyyyyyyy")

        reference_df = df[(df["datetime"] >= ref_start_dt) & (df["datetime"] <= ref_end_dt)]
        current_df = df[(df["datetime"] >= cur_start_dt) & (df["datetime"] <= cur_end_dt)]

        self.reference_df = reference_df
        self.current_df = current_df
        # print(reference_df, current_df)
    ...
    def generate_full_drift_report(self):
        ...
        return html, drift_dict

    # json_path, upload_result
...
- Briefly describe how the method (e.g., KS test, PSI, Wasserstein distance) detects drift
    ...
    def _invoke_claude(self, prompt):
        ...
        try:
            ...
            logging.exception("Claude invocation failed")
            return "Claude invocation failed due to an internal error."
