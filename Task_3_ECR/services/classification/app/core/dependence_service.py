...
class DependenceService:
    ...
    def partial_dependence(
        ...
        for v in grid:
            ...
            # Get predictions for all instances and average
            y_pred = self.base.safe_predict_proba(X_modified)[:, 1]  # Positive class
    ...
    def shap_dependence(
        ...
        payload: Dict[str, Any] = {
            "feature": feature_name,
            "feature_values": (
                feature_vals.astype(float).tolist()
                if pd.api.types.is_numeric_dtype(feature_vals.dtype)
                else feature_vals.astype(str).tolist()
            ),
        ...
        plot_metadata = {
            ...
            "x_label": feature_name,
            "y_label": f"SHAP value for {feature_name}",
        ...
        if color_by and color_by in self.base.feature_names:
            ...
            payload["color_values"] = (
                color_vals.astype(float).tolist()
                if pd.api.types.is_numeric_dtype(color_vals.dtype)
                else color_vals.astype(str).tolist()
            )
            plot_metadata["color_scale_label"] = color_by
    ...
    def ice_plot(
        ...
        """Compute Individual Conditional Expectation (ICE) plot data with average PDP line."""
        self.base._ensure_loaded_with_payload(payload)
        ...
        curves = []
        all_predictions = []  # Store all predictions for each grid point
        ...
        for i in sample_idx:
            ...
            for val in grid:
                ...
                pred = float(self.base.safe_predict_proba(instance_df)[0, 1])
            ...
            all_predictions.append(curve_preds)

        # Compute average PDP line (mean of ICE lines for each feature value)
        if all_predictions:
            avg_predictions = np.mean(all_predictions, axis=0).tolist()
            pdp_line = {
                "x": grid.tolist(),
                "y": avg_predictions,
                "color": "#FF6B35",  # Bold orange color for distinction
                "label": "Average PDP",
            }
        else:
            pdp_line = None
        ...
        legend_entries = [f"Instance {i}" for i in sample_idx]
        if pdp_line:
            legend_entries.append("Average PDP")
