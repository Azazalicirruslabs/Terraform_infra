...
@router.post("/analysis/explain-instance/{instance_idx}", tags=["Analysis"])
async def explain_instance(
    ...
    try:
        ...
        try:
            data_preview = model_service.explain_instance(model_overview_payload, instance_idx)
        except IndexError:
            raise HTTPException(status_code=400, detail=f"❌ Instance index {instance_idx} is out of range.")
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"⚠️ Failed to generate instance explanation: {str(e)}")
        ...
        return handle_request(model_service.explain_instance, model_overview_payload, instance_idx)
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"⚠️ Failed to explain instance due to an unexpected internal error: {str(e)}"
        )
