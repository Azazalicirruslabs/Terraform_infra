...
@router.post("/api/threshold-analysis", tags=["Classification"])
async def post_threshold_analysis(
    url_payload: RequestPayload,
    num_thresholds: float = 0.5,
    ...
    try:
        ...
        # ✅ Validate num_thresholds
        if not isinstance(num_thresholds, (int, float)) or num_thresholds <= 0:
            raise HTTPException(status_code=400, detail="❌ 'num_thresholds' must be a positive number.")
        ...
        try:
            data_preview_response = handle_request(
                model_service.threshold_analysis, model_overview_payload, num_thresholds
            )
            data_preview = json.loads(data_preview_response.body)
        except ValueError as ve:
            raise HTTPException(
                status_code=400,
                detail=f"❌ Failed to perform threshold analysis due to invalid data or parameters: {str(ve)}",
            )
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"⚠️ Failed to compute threshold analysis: {str(e)}")
        ...
        if not existing:
            new_entry = AnalysisResult(
                ...
                analysis_tab="threshold-analysis",
                project_id="N/A",
        ...
        raise HTTPException(
            status_code=500, detail=f"⚠️ Unexpected internal error while performing threshold analysis: {str(e)}"
        )
