...
@router.post("/api/ice-plot", tags=["Dependence"])
    ...
    try:
        ...
        try:
            data_preview_response = handle_request(
                model_service.ice_plot, model_overview_payload, feature, num_points, num_instances
            )
            ...
            raise HTTPException(status_code=500, detail=f"⚠️ Failed to compute ICE plot: {str(e)}")
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"⚠️ Unexpected internal error while performing ICE plot analysis: {str(e)}"
        )
